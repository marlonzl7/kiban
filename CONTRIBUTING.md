...
| `install.<pm>.steps[].sudo` | Sim | `true` se o comando precisa rodar com sudo. Não pode ser `true` junto com `shell: true` (veja [Regras extras para steps com `shell: true`](#regras-extras-para-steps-com-shell-true)) |
| `install.<pm>.steps[].shell` | Não (default `false`) | `true` se o comando precisa de interpretação de shell (pipe, subshell, redirecionamento, heredoc). Use apenas quando estritamente necessário |
...
### Regras extras para steps com `shell: true`

Por padrão, um `cmd` é executado literalmente (sem interpretação de
shell), o que já elimina boa parte dos riscos abaixo. Alguns comandos,
porém, exigem pipe, subshell, redirecionamento ou heredoc (ex:
instalação via repositório oficial do Docker) — nesses casos, o step
declara `shell: true`, e passa a valer um padrão de revisão mais
rigoroso, tanto para quem abre o PR quanto para quem revisa.

**Para quem contribui:** um `Step` não pode declarar `shell: true` e
`sudo: true` ao mesmo tempo — isso é validado automaticamente e a
instalação falha antes de começar, com uma mensagem apontando qual
ferramenta e step estão em conflito. Em modo `shell: true`, o `sudo`
deve estar explícito dentro do próprio texto do `cmd`, aplicado só ao
comando que de fato precisa de privilégio elevado (ex:
`curl ... | sudo tee ...`), em vez de todo o pipeline — isso segue o
princípio de privilégio mínimo: cada parte do comando tem só o
privilégio que sua própria ação exige.

Além disso, um PR com um step `shell: true` contendo qualquer um dos
padrões abaixo será rejeitado ou terá ajuste solicitado, mesmo que o
comando funcione corretamente:

- **Conteúdo codificado** (base64, hex, ou qualquer formato que exija
  decodificação para ser lido). Escreva o comando em texto plano —
  se a fonte oficial da ferramenta usa codificação, decodifique e
...
**Para quem revisa:** além de verificar os pontos acima, dedique
atenção extra a:

- **URLs**: copie a URL para fora do editor e leia o domínio caractere
  por caractere, checando por typosquatting (ex:
  `docker.com.evil-mirror.io`) ou caracteres visualmente parecidos
  (homóglifos, ex: um "o" cirílico no lugar do "o" latino).
- **Redirecionamento (`>`, `>>`) ou escrita em arquivo do sistema**:
  confirme que o destino é exatamente o esperado pelo processo oficial
  da ferramenta, sem paths estranhos ou relativos (`../../etc/...`).
- **PRs longos que replicam documentação oficial**: compare linha por
  linha com a fonte oficial real, em vez de confiar que "parece
  familiar" — é justamente nesses PRs que uma linha extra ou alterada
  passa mais despercebida.
- Na dúvida, teste o step em uma VM isolada antes de aprovar, em vez de
  confiar apenas na leitura do código.